...
#          p.modified_venues = modifiedVenues
  ...
  def getNewProgramItems
    clause = addClause(nil,'programme_items.visibility_id != ?', Visibility['None'].id) # only get those that are not admin-only
  ...
  def getNewChildren
    clause = addClause(nil,'programme_items.visibility_id != ?', Visibility['None'].id) # only get those that are not admin-only
    clause = addClause(clause,'programme_items.id not in (select publications.original_id from publications where publications.original_type = ?)', 'ProgrammeItem')
    clause = addClause(clause,'programme_items.parent_id is not null AND parents_programme_items.visibility_id != ? ', Visibility['None'].id)
  ...
  def getModifiedProgramItems
    clause = addClause(nil,'visibility_id != ?', Visibility['None']) # only get those that are not admin-only
  ...
  def getUnpublishedItems
    PublishedProgrammeItem.joins("left outer join publications on (publications.published_id = published_programme_items.id AND publications.published_type = 'PublishedProgrammeItem')").
      ...
      where("(programme_items.visibility_id = #{Visibility['None'].id} AND publications.published_id is not null AND published_programme_items.parent_id is null)")
  end

  def getUnpublishedSubItems
    PublishedProgrammeItem.joins("left outer join publications on (publications.published_id = published_programme_items.id AND publications.published_type = 'PublishedProgrammeItem')").
      joins("join programme_items on programme_items.id = publications.original_id and publications.original_type = 'ProgrammeItem'").
      where("(programme_items.visibility_id = #{Visibility['None'].id} AND publications.published_id is not null AND published_programme_items.parent_id is not null)")
  end
  ...
  def publishRooms(rooms)
    ...
    rooms.each do |srcRoom|
      nbrProcessed += 1 if publishRoom(srcRoom)
    end
  ...
  def publishVenues(venues)
    ...
    venues.each do |srcVenue|
      nbrProcessed += 1 if publishVenue(srcVenue)
    end

    nbrProcessed
  end

  def publishRoom(src_room)
    # 1. find out if the room is already published
    pub_room = src_room.published

    # 2. if not then publish it
    if pub_room.nil?
      pub_room = PublishedRoom.new
      pub_room.sort_order = src_room.sort_order
      pub_room.original = src_room
      pub_room.publication.publication_date = DateTime.current
      pub_room.publication.user = @current_user

      pub_room = set_translated_attrs(%w[name], src_room, pub_room)

      # we will need the venues as well
      pub_venue = publishVenue(src_room.venue)
      pub_room.published_venue = pub_venue

      pub_room.save
    elsif src_room.updated_at > pub_room.updated_at
      # If the room is published check to see if there is a name change/update
  ...
  def publishVenue(src_venue)
    pub_venue = src_venue.published

    if pub_venue.nil?
      pub_venue = PublishedVenue.new
      pub_venue.sort_order = src_venue.sort_order
      pub_venue.original = src_venue
      pub_venue.publication.publication_date = DateTime.current
      pub_venue.publication.user = @current_user
  ...
  # Takes a list of attribute names and the source and destination objects
  def set_translated_attrs(attrs, src, dest)
    return unless attrs.is_a?(Array) && attrs.any?
    # The translated fields also need to get copied ...
    # for each locale get the translation if it exists etc
    UISettingsService.getSiteLanguages.each do |lang|
      I18n.with_locale(lang) do
        # Check and see if the src model and dest model respond to the
        # getter and setter methods generated by globalize_accessors
        # before assigning the source value to the destination attribute
        attrs.each do |attr|
          next unless dest.respond_to?("#{attr}_#{lang}=") &&
                      src.respond_to?("#{attr}_#{lang}")

          dest.public_send(
            "#{attr}_#{lang}=",
            src.public_send("#{attr}_#{lang}")
          )
        end
      end
    end
