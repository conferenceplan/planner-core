    ...
    if dest.is_a?(PublishedProgrammeItem)
      ...
      UISettingsService.getSiteLanguages.each do |lang|
        I18n.with_locale(lang) do
          ...
          if dest.respond_to?("title_#{lang}=") &&
             src.respond_to?("title_#{lang}")
            dest.public_send(
              "title_#{lang}=",
              src.public_send("title_#{lang}")
            )
          end
          if dest.respond_to?("short_title_#{lang}=") &&
             src.respond_to?("short_title_#{lang}")
            dest.public_send(
              "short_title_#{lang}=",
              src.public_send("short_title_#{lang}")
            )
          end
          if dest.respond_to?("description_#{lang}=") &&
             src.respond_to?("description_#{lang}")
            dest.public_send(
              "description_#{lang}=",
              src.public_send("description_#{lang}")
            )
          end
        end
      end
