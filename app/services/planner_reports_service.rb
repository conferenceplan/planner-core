...
module PlannerReportsService
  ...
  def self.findParticipantsWithNoBios
    ...
    conditions = ["(programme_item_assignments.role_id in (?)) AND (programme_items.target_audience_id != #{TargetAudience['Admin'].id}) AND (edited_bios.id is null OR edited_bios.bio is null OR edited_bios.bio = '')",
  ...
  def self.findPanelistsAndBios(order_by)
    ...
    cndStr += " AND (programme_items.target_audience_id != #{TargetAudience['Admin'].id})"
  ...
  def self.findProgramItemsByTimeAndRoom
    TimeSlot.joins([:programme_items]).
              ...
              references([
                 ...
                 ]).              where("programme_items.target_audience_id != #{TargetAudience['Admin'].id} and time_slots.start is not NULL").
              where(self.constraints()).
              order("time_slots.start, venues.sort_order, rooms.sort_order") #
  end
