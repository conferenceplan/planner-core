class Participants::PendingListCell < Cell::Rails

  def display(args = {})
    @place = args.has_key?(:place) ? args[:place] : "pending-list"
    render
  end
  ...
  def javascript(args)
    ...
    @baseUrl = args.has_key?(:baseUrl) ? args[:baseUrl] : "pending_import_people"
    ...
    @cardView = args.has_key?(:cardView) ? args[:cardView] : true
