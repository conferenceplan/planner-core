    ...
    @rooms.each do |room|
        ...
        room.published_room_item_assignments.each do |assignment|
            if current_day != assignment.day
                ...
                pdf.text  '<b>' + room.name + ' - ' + room.published_venue.name + ' <b>' + (Time.zone.parse(SiteConfig.first.start_date.to_s) + assignment.day.days).strftime('%A') + '</b>', :size => 30, :inline_format => true, :align => :center
                pdf.move_down 1.in
                pdf.font_size 16
            end
