    ...
    @rooms.each do |room|
        ...
        room.published_room_item_assignments.each do |assignment|
            if (current_day != assignment.day) || @one_per_page
                ...
                pdf.text  '<b>' + room.name + ' - ' + room.published_venue.name + '<br>' + (Time.zone.parse(SiteConfig.first.start_date.beginning_of_day.to_s) + assignment.day.days).strftime('%A') + '</b>', :size => 16, :inline_format => true, :align => :center, :fallback_fonts => planner_fallback_fonts
