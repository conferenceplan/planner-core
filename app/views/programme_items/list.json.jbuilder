...
json.rows @items do |item|
    ...
    json.start_time_str      Time.zone.parse((item.start_time).to_s).strftime('%m/%d/%Y %H:%M:%S')
    json.end_time            item.end_time
    json.end_time_str        Time.zone.parse((item.end_time).to_s).strftime('%m/%d/%Y %H:%M:%S')
