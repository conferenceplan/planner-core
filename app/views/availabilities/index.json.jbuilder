...
if @availableDates
    json.dates do
        ...
        json.start_time_str (@availableDates.start_time ? @availableDates.start_time.strftime('%A, %B %e %Y,%l:%M %p') : nil)
...
json.times @excludedTimesGroup.each do |time|
    ...
    json.end            time.end
    json.start_str      (time.start ? time.start.strftime('%A, %B %e %Y,%l:%M %p') : nil)
    json.end_str        (time.end ? time.end.strftime('%A, %B %e %Y,%l:%M %p') : nil)
end
